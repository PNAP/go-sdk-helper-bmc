...
func (command *CreateServerPrivateNetworkCommand) Execute() (*bmcapiclient.ServerPrivateNetwork, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("CreateServerPrivateNetworkCommand Returned Code %v Message: %s Validation Errors: %s", httpResponse.StatusCode, response.Message, response.ValidationErrors)
