...
func (command *CreateServerCommand) Execute() (*bmcapiclient.Server, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("CreateServerCommand Returned Code %v Message: %s Validation Errors: %s", httpResponse.StatusCode, response.Message, response.ValidationErrors)
