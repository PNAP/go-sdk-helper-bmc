...
func (command *GetServersCommand) Execute() ([]bmcapiclient.Server, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("GetServersCommand Returned Code %v Message: %s Validation Errors: %s", httpResponse.StatusCode, response.Message, response.ValidationErrors)
