...
func (command *ResetServerCommand) Execute() (*bmcapiclient.ResetResult, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("ResetServerCommand Returned Code %v Message: %s Validation Errors: %s", httpResponse.StatusCode, response.Message, response.ValidationErrors)
