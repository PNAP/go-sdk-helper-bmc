...
func (command *UpdateSshKeyCommand) Execute() (*bmcapiclient.SshKey, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("UpdateSshKeyCommand Returned Code %v Message: %s Validation Errors: %s", httpResponse.StatusCode, response.Message, response.ValidationErrors)
