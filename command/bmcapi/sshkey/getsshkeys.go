...
func (command *GetSshKeysCommand) Execute() ([]bmcapiclient.SshKey, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("GetSshKeysCommand Returned Code %v Message: %s Validation Errors: %s", httpResponse.StatusCode, response.Message, response.ValidationErrors)
